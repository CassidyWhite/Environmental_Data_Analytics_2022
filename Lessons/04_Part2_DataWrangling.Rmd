...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #select the whole dataset
  mutate(month = month(sampledate)) %>% #add column for month
  select(lakeid:daynum, month, sampledate:comments) %>% #select columns you want to keep and drop all rows with an NA in the depth column
  drop_na(depth)

view(NTL.nutrient.data.PeterPaul)
summary(NTL.nutrient.data.PeterPaul$depth) #look at the summary to confirm all NAs were dropped from depth
...
view(NTL.nutrient.data.PeterPaul.subset)
...
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) #remove NA from concentration column
...
Litter.2016 <- rbind(Litter.June2016, Litter.July2016, Litter.August2016)
```
...
```{r}
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE) #will list all the files in this folder
LitterFiles

Litter <- LitterFiles %>%
  ldply(read.csv) #read all 20 files together
```
...
NTL.PeterPaul.summaries <- 
  ...
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #remove nas for 3 columns
  summarise(meantemp = mean(temperature_C), #summarize for a specific lake and a specific month (this is why we are using groupby)
            sdtemp = sd(temperature_C), 
            meanTN = mean(tn_ug), 
            sdTN = sd(tn_ug), 
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
#where did the lakename and month come into play? We used group_by and I thought we would then get multiple summaries for each month for each lake
