...
`==` setting equal to a specific value
`!=` find values that are different than a specific number
`<`
`<=`
`>`
`>=`
`&` when you have multiple conditions that you want to be true
`|` when you have multiple conditions and only one or the other needs to be true
...
#Remember matrices are listed [row,column]
#Matrix filtering can only be used for numeric values
...
#could have a previously defined vector for lakename %in% c(...)
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #comma is interpretted as a combination of both, same as "&"
...
NTL.phys.data.1999<-NTL.phys.data[NTL.phys.data$year4 == 1999,]

# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.1990.1999<-NTL.phys.data[NTL.phys.data$year4 == 1990:1999,]
view(NTL.phys.data.1990.1999)
```
...
view(NTL.phys.data.depth.ascending)
# Exercise: 
...
view(NTL.phys.data.temperature.descending)
```
### Select
...
#selects lakename column and then columns sampledate through temperature_C
view(NTL.phys.data.temps)
```
### Mutate
...
#adds a new column where temperature is converted to Fahrenheit
view(NTL.phys.data.temps)
```
...
#select can be used to pull columns or to reorder columns

# find out the start and end dates of the dataset
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613]) #if dates are already in ascending order
...
```{r}
NTL.phys.data.processed <- #rename the new dataset
  NTL.phys.data %>% #select the entire dataset
  filter(lakename == "Paul Lake" | lakename == "Peter Lake") %>% #filter for Peter and Paul Lakes
  select(lakename, sampledate:temperature_C) %>% #select rows with lakename and temperature information
  mutate(temperature_F = (temperature_C*9/5) + 32) #add a new column where temperature in Fahrenheit is calculated
