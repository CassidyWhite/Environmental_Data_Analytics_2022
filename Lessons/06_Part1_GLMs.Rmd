...
par(mfrow = c(2,2), mar=c(4,4,4,4)) #mfrow tells R to split the screen and plot the 4 graphs together and mar says how big to make them
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) #eliminate 0s to run the log and eliminate the outlier
...
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
  geom_smooth(method = "lm") + #adds a line for the linear relationship between x and y
  scale_y_log10() + #does the log of irradiance water
  geom_point() 
...
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point() +
    scale_y_log10() +  #log transformed values for irradiance Water
    geom_smooth(method = 'lm', se = FALSE, color = "black") #se = FALSE removes the confidence interval around the line
print(irradiancebydepth2)
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit()
PeterPaulCorr <- cor(PeterPaulnutrients)  #how to find which variables are highly correlated with other variables
corrplot(PeterPaulCorr, method = "ellipse") #the thiner the circle the stronger the correlation
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #just print the upper diagonal because the upper and lower are the same
```
...
#low AIC is best
step(TPAIC) #analyze addition of each variable in a stepwise fashion to find the combination of variables that makes the best explanation of the variation in y (depth)
