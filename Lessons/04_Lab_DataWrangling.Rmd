...
NTL.phys.data.Tuesday.90s <- NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 %in% c(1990:1999))
```
Question: Why don't we filter using row numbers?

> Answer: Row numbers change when we manipulate the data or use different data sets. Using row values to filter makes sure you are telling the computer to do the same thing to code each time. To select the same variables even if row numbers change.
...
NTL.phys.data.Tuesday.90s.July<-NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 %in% c(1990:1999)) %>%
...
NTL.phys.data.Tuesday.summary <- NTL.phys.data.Tuesday.90s.July %>%
  filter(depth == 0) %>%
...
NTL.phys.data.gathered<-pivot_longer(NTL.phys.data,irradianceWater:irradianceDeck, names_to = "Location", values_to = "Irradiance")
