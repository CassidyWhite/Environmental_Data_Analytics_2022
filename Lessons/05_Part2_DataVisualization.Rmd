...
O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray() #default ggplot theme so nothing changed
print(O3plot1)
...
O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() #gray lines with white background
print(O3plot3)

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() #white background with black points but no background lines
print(O3plot4)
```
...
theme_set(mytheme) #set your own theme as the default in your R session
...
NvsP2 <-
  ...
  scale_shape_manual(values = c(15, 17, 18, 19, 20)) +
