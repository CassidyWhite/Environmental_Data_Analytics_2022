...
#draws a bounding box and gives you the lower left and upper right coordinates of the box around the points
st_bbox(counties_sf)
...
huc_sf<-st_read("./Data/Spatial/NCHUC8.shp")
...
upperNeuse_sf<-st_read("./Data/Spatial/NCHUC8.shp") %>%
  filter(SUBBASIN == "Upper Neuse")

#Create a map
mapview(huc_sf, col.regions = "blue") +
  mapview(upperNeuse_sf, col.regions = "green")
```
...
nc_powerplants_sf <- powerplants_sf %>% 
  filter(STATE == "NC") 
mapview(nc_powerplants_sf$geometry)
...
#chooses a color pallet for a continuum for number of people in poverty and then adds to the map the pm2.5 sites with points sized by max pm2.5
...
windenergy<-read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", colClasses = c('HUC_12' = 'factor')) %>%
...
huc8_sf_utm <-     st_transform(huc_sf, crs = 26917)
huc2_utm <-        st_transform(DWQ_Basin, crs = 26917)
```
...
#x="." means whatever you are filtering from, so counties_sf_utm in our case

#View the result
myMap + mapview(neuse_intersect_2, alpha.regions = 0)

#Actually intersect the counties (not just select those that intersect)
#first input (neuse_sf) is the cookie cutter and st_intersection() is the dough you are cutting from
...
resultMask_30km<-st_intersects(counties_sf_utm, site_30km, sparse = FALSE)
...
huc8s_WGS84 <- st_transform(huc_sf,c=4326)
huc2_WGS84<- st_transform(DWQ_Basin,c=4326)
...
leaflet() %>% addTiles() %>% 
  ...
  addPolygons(data=huc_sf,weight=1)
```
