...
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue") #the color needs to be added outside the aesthetic
print(O3plot2)

# Add additional variables
# How could you automatically assign a marker color to a variable? 
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) + #shape decides the shape of the dot and color is set to vary based on the variable 'site name'
  geom_point()
print(PMplot)

# Separate plot with facets (by site name)
PMplot.faceted <-
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year))) +
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #instead of coloring the sites with different colors, we are creating one plot for each site
print(PMplot.faceted)

# Separate plot with facets (by year)
PMplot.faceted <-
  ggplot(EPAair, aes(x = Month, y = PM2.5, color = Site.Name)) + #instead of identifying the years with different shapes, we are creating one plot for each year
  geom_point() +
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
                  Site.Name == "Bryson City"), #we only want to plot information for these three sites
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #create a new plot for each site for each year; columns are years and rows are site names
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm) + #find a line to represent the relationship between the two nutrients
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? The color is the contour of the shapes and fill is the inside of the shape (box plot)
print(Nutrientplot3)

# Dot plot
Nutrientplot4 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, #binaxis means I'm stacking the values over y
               stackdir = "center", position = "dodge", dotsize = 2) #stackdir says I'm stacking the points based on their center position
                                                                    #position = dodge tells values at the same level to go to the side rather than overlaying
print(Nutrientplot4)

# Violin plot
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_violin(aes(color = nutrient)) #you can see the density of observations and which have more variation
print(Nutrientplot5)

# Frequency polygons
# Using a tidy dataset
#This is not a good example of how to do a frequency plot. There is an easier way.
Nutrientplot6 <-
...
#This shows why it's important to have that 'gathered' dataset from pivot_longer
Nutrientplot7 <-   
...
# Ridgeline plot
#This is the best option for a frequency plot 
Nutrientplot6 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(y = nutrient, x = concentration)) +
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) #different colors for each lake
print(Nutrientplot6)
```
