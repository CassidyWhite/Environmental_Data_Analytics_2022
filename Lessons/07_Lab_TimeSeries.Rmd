...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #Similar to text to column function in Excel
  mutate( Date = my(paste0(Month,"-",Year))) %>% 
  select(Date,Rainfall) # we created columns Null and Null2 for the space at the beginning of the column and the Average at the end of the text in the statistics column and then we get rid of them by only selecting the columns we want in the final dataset
...
Rainfall.ts<-ts(BRA_complete$Rainfall, start = c(fmonth, fyear), frequency = 12)
Temperature.ts<-ts(BRA_complete$Temperature_C, start = c(fmonth, fyear), frequency = 12)
```
...
#stl decomposes the data in seasonal, trend, and random (remainder) terms

Rainfall.Decomposed<-stl(Rainfall.ts, s.window = "periodic")
Temperature.Decomposed<-stl(Temperature.ts, s.window = "periodic")

plot(Rainfall.Decomposed)
plot(Temperature.Decomposed)

#Plot 1: Observed Data
#Plot 2: Seasonal Data
#Plot 3: Trend Component of the Data
#Plot 4: Random Component of the Data
...
Rainfall_Components <- as.data.frame(Rainfall.Decomposed$time.series[,1:3])
Temperature_Components<-as.data.frame(Temperature.Decomposed$time.series[,1:3])
...
Temperature_Components<-mutate(Temperature_Components,
...
ggplot(Rainfall_Components) +
  ...
  geom_hline(yintercept = 0, lty = 2) + #geometric horizontol line at y=0
  ylab(expression("Raifnall"))
...
Rainfall.trend<-Kendall::SeasonalMannKendall(Rainfall.ts)
Temperature.trend<-Kendall::SeasonalMannKendall(Temperature.ts)

Rainfall.trend
summary(Rainfall.trend)

Temperature.trend
summary(Rainfall.trend)

Rainfall.trend2<-trend::smk.test(Rainfall.ts)
Temperature.trend2<-trend::smk.test(Rainfall.ts)

Rainfall.trend2
summary(Rainfall.trend2)

Temperature.trend2
summary(Temperature.trend2)
