...
Litter.Totals <- Litter %>%
  ...
  summarise(dryMass = sum(dryMass)) #grabbed the total mass for all plots that were the same and had data collected on the same day and of the same class

summary(Litter.Totals)
#check all value of plotID
summary(Litter.Totals$plotID) #you can see some sites have more samples than others but we can still run the test
...
#P<.05 means they are normally distributed
...
# Test for equal variance - we don't want equal variance
...
#null hypothesis is that means are the same across all the populations

# Format ANOVA as lm
Litter.Totals.anova2 <- lm(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova2)

#main difference between Anova and Lm output is the summary. Lm will have one row for each level of the categorical variable.
...
#calls the result of an anova test
...
Litter.Totals.plot <- ggplot(Litter.Totals, aes(x = plotID, y = dryMass)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + #angle text because site ID is long
...
Litter.groups <- HSD.test(Litter.anova.2way5, "Litter.interaction", group = TRUE) #which interactions lead to means that are not statistically different
Litter.groups
