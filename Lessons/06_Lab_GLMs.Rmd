...
> We are trying to prove an assumption we have about a sample.
...
```{r}
shapiro.test(EPAair$Ozone[EPAair$Year == 2018]) #filtering for a single year and seeing if levels are the same within that year
...
#1
Density.plot<-ggplot(EPAair, aes(x= Ozone, fill = as.factor(Year)))+
...
#2
Box.plot<-ggplot(EPAair, aes(x= as.factor(Year), y=Ozone))+
  geom_boxplot()+
  stat_summary(geom = "text", fun.y = max, vjust = -1, size = 4, #-1 has it go above the point
               label = c("b", "a"))+
  labs(x = "Year", y= "Ozone AQI Value", fill = "Year")+
  ylim(0,150)
print(Box.plot)

#3
Violin.plot<-ggplot(EPAair, aes(x= as.factor(Year), y=Ozone))+
...
O3.regression <- lm(EPAair$PM2.5 ~ EPAair$Ozone)
summary(O3.regression)

#R^2 = 0.1314
#p.value = <0.5
#This is a significant relationship but Ozone does not explain all of the variation in PM2.5.

#Exercise 4: Build a scatterplot. Add a line and standard error for the linear regression. 
scatter.plot<-ggplot(EPAair, aes(x=Ozone, y = PM2.5))+
  geom_point()+
...
EPAairAIC<-lm(data = EPAair.subset, PM2.5 ~ meanLat+ meanLng+ Month + Year + Ozone)
step(EPAairAIC)
...
EPAairModel<-lm(data = EPAair.subset, PM2.5 ~ meanLng + meanLat + Ozone + Month)
summary(EPAairModel)
