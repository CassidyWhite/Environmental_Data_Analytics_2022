...
ggplot(USGS.flow.data, aes(x = discharge.mean.approval)) +
  geom_bar() #bar charts are for categorical data. Count by category
```
...
#histograms are for numerical data. Count by numerical interval.
#
ggplot(USGS.flow.data) +
...
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  ...
  scale_x_continuous(limits = c(0, 500)) #forces x axis to go from 0 to 500 so we can focus on where most of the data is
...
#
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean), bins = 50, color = "darkgreen",  lty = 2) +
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "red") +
  geom_freqpoly(aes(x = gage.height.max), bins = 50, color = "blue") +
...
#showing the distribution in values across gage.height.mean
#at low values of gage height, there is a tighter distribution of values
#
ggplot(USGS.flow.data) +
  geom_violin(aes(x = gage.height.mean.approval, y = gage.height.mean), 
              draw_quantiles = c(0.25, 0.5, 0.75))
#shows the range of values and distribution of values within the interquartile ranges
```
