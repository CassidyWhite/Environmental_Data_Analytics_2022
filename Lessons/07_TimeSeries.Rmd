...
#Questions: 
#1. Why would we want to detrend the data? That sounds like twisting the data to me.
#2. When runnign the Seasonal MannKendall test, if only one season were significant, would it still say there is a trend in the data? What's the cutoff?
...
EnoDischarge.early <- EnoDischarge.complete %>%
  filter(datetime < as.Date("1985-01-01")) #filters for dates before 1985

EnoDischarge.late <- EnoDischarge.complete %>%
  filter(datetime > as.Date("1985-01-01")) #filters for dates after 1985
```
...
wind_data_clean <- 
  wind_data %>% 
  mutate( AWND.clean = zoo::na.approx(AWND) ) #zoo::na.approx is calling the package that the function comes from. If the NA is at the beginning or the end, you can use na.omit, but if the na is in the middle you need to replace the NA which is why we use na.approx
...
f_month <- month(first(wind_data_clean$DATE)) #defining the first month from the first date as the start point
f_year <- year(first(wind_data_clean$DATE)) #defining the year from the first date as the start point
...
#Significant p value so this test shows the data is not stationary. There is a trend. Null hypothesis is that the data is stationary.
...
#Shows you for which seasons you have a significant trend.
