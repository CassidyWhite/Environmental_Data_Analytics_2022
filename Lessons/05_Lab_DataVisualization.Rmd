...
plot1<-ggplot(subset(PeterPaul.chem.nutrients, depth == 0), aes(x = daynum, y = temperature_C, color = year4))+
  geom_point()+
  facet_wrap(vars(lakename), nrow=2)+
  labs(x="Day of the Year", y="Surface Temperature (C)")
print(plot1)
...
plot2<-ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C, color=depth))+
  geom_point(size = 0.5)+
...
plot3<-ggplot(EPAair, aes(x=PM2.5, y=Ozone, color = SITE_LATITUDE))+
