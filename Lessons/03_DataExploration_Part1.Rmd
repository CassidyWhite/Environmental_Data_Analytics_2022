...
> Answer: All packages associated with that package are installed.
...
> ANSWER: This file is found in the Metadata folder within the Data folder.The information found in this file is reflective of best practices for working with data because it says where the data came from, the units of the data, the date range for the data, the day the data was downloaded, and a general description of the data, among other information. This provides information about the data that may not be known from looking at the data itself that can help with reproducability.
...
str(USGS.flow.data) #shows the structure of the dataset
dim(USGS.flow.data) #shows the dimensions of the dataset
length(USGS.flow.data)#gives the length of the dataset

head(USGS.flow.data) #gives the first 6 rows of the dataset
head(USGS.flow.data, 10) #gives the first 10 rows of a dataset
tail(USGS.flow.data, 5) #gives the last 5 rows of the dataset
USGS.flow.data[30000:30005, c(3, 8, 14)] #gives rows 30000-30005 and columns 3, 8, and 14 of the dataset

class(USGS.flow.data$datetime) #can look at the class of a specific column
...
> Answer: NA is imported in the empty cell.
...
> Answer: You may want to omit NAs if you want to compute a mean.
...
create.early.dates <- (function(d) {
       paste0(ifelse(d > 191226,"19","20"),d) #change years incorrectly labeled as "20.." to "19.."
       })
#
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime) #rename dataset with those corrected dates

#
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%Y%m%d") #reclassify datetime as Date data type instead of character
