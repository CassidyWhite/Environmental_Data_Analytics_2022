...
#4
withdrawals_df<-data.frame("Month" = c("Jan", "May", "Sep", "Feb", "Jun", "Oct", "Mar", "Jul", "Nov", "Apr", "Aug", "Dec"),
                          "Year" = rep(2020,12),
...
scrape.it <- function(the_year, the_pswid){
   ...
   withdrawals_function_df<-data.frame(
    ...
    "Max Daily Withdrawals" = max.withdrawals.mgd, 
...
ggplot(df, aes(x=System.Name, y = Max.Daily.Withdrawals))+
  geom_boxplot()
```
...
ggplot(dfAsh, aes(x=month(Date), y=Max.Daily.Withdrawals))+
  geom_point()+
  xlim(0,12)+
